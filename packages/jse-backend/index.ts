...
const PORT: number = !process.env.LISTEN_PORT ? 3001 : Number(process.env.LISTEN_PORT);
...
function getAllJobs() {
  try {
    ...
    scrapeAllJobs('https://www.indeed.com/jobs?q=Front+end+engineer&sc=0kf%3Ajt%28internship%29%3B', true).then(res => {
      result = res;
      jobsFromDB = res;
